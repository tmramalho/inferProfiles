...
class MLLiu(object):
	...
	def fitFlyLine(self, i, n = 10):
		...
		start = time.clock()
		self.trainDifferentialEvolution(nGenerations = 4000)
		np.save("results/mlbest{0:02d}.npy".format(i), ml.best)
		#self.best = np.load("results/mlbest.npy")
		print "Done in", (time.clock() - start)*1000
		
	def getScatterData(self, i):
		x, y = self.dataset.returnPointCloudForFlyLine(i, True, 0, 1)
		return (x, y)
...
if __name__ == '__main__':
	...
	xmi = 0.05
	xma = 1
	ml = MLLiu(fly, xmin=xmi, xmax=xma, mpen=1e1, spen=1e-1)
	dosage = []
	params = []
	for line in range(fly.numLines):
		...
		params.append([res.x[0], fi(res.x[0])])
		...
		plt.ylim([0, 0.06])
		plt.xlim([0,1])
		plt.savefig('results/liuFisher{0:02d}.pdf'.format(line+1))
	params = np.array(params)
	plt.subplot(211)
	plt.scatter(dosage, params[:, 0])
	plt.ylim([0,1])
	plt.subplot(212)
	plt.scatter(dosage, np.sqrt(params[:, 1]))
