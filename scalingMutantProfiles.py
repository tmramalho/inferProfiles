...
class ScalingContainerMutant(object):
		...
		for dc in self.data_containers:
			for i, g in enumerate(dc['Genename']):
				...
				for b in xrange(len(bins)-1):
					...
					plt.savefig(ensure_dir("plots/mt/scaling_{0}_{1}_{2}.pdf".format(i, g, b)))
					plt.clf()
					plt.plot(x, dc['meas'][:,i,1,r_list[0]])
					plt.plot(x, dc['meas'][:,i,0,r_list[0]])
					plt.savefig(ensure_dir("plots/mt/debug_{0}_{1}_{2}.pdf".format(i, g, b)))
					plt.clf()
		...
		for li in xrange(2):
			for dc in self.data_containers:
				...
				for i, g in enumerate(dc['Genename']):
					...
					plt.savefig(ensure_dir("plots/mt/timeplot_{0}_{1}_{2}.pdf".format(dc['Flylinename'], g, li)))
					plt.clf()
				...
				for i, g in enumerate(dc['Genename']):
					...
					for bi in xrange(n_bins):
						...
						for oi in xrange(3):
							...
							plt.savefig(ensure_dir("plots/mt/pca_{0}_{1}_{2}_{3}_{4}.pdf".format(li, dc['Flylinename'], g, oi, bi)))
							plt.clf()
					...
					leg = plt.legend(loc="lower left", fontsize = 8)
					leg.get_frame().set_alpha(0.5)
					plt.savefig(ensure_dir("plots/mt/tpca_{0}_{1}_{2}.pdf".format(dc['Flylinename'], g, li)))
					plt.clf()
				...
				for i, g in enumerate(dc['Genename']):
					...
					for bi in xrange(2):
						...
						for oi in xrange(3):
							...
							plt.title('pc2')
							plt.suptitle('{3} {0} a{1:.1f} n{2}'.format(g, np.average(dc['age'][pos]), y.shape[0], names[oi]))
							plt.savefig(ensure_dir("plots/mt/gr_pca_{0}_{1}_{2}_{3}_{4}.pdf".format(li, dc['Flylinename'], g, oi, bi)))
							plt.clf()
					plt.figure(figsize=(5,12), tight_layout=True)
					plt.subplot(411)
					for oi in xrange(3):
					...
					leg = plt.legend(loc="lower left", fontsize = 8)
					leg.get_frame().set_alpha(0.5)
					plt.savefig(ensure_dir("plots/mt/gr_tpca_{0}_{1}_{2}.pdf".format(dc['Flylinename'], g, li)))
					plt.clf()
				...
				for i, g in enumerate(dc['Genename']):
					...
					for bi in xrange(n_bins):
						...
						for oi in xrange(3):
							y = np.array(y_arr[oi])
							L = np.array(ll[oi])
							print '{0} {1} {2}'.format(g, names[oi], dc['Flylinename'])
							print y.shape
							if len(y.shape) < 2:
								print "Few samples!"
								vals[bi,0,:,li,oi] = [1,0,0,0]
							...
							plt.plot(x, lower_y.T)
							plt.xlabel('x/L')
							plt.ylabel('conc.')
							plt.subplot(322)
							x = np.linspace(0,1,upper_y.shape[1])
							plt.plot(x, upper_y.T)
							plt.xlabel('x/L')
							plt.ylabel('conc.')
							plt.subplot(323)
							x = np.linspace(0,1,lower_y.shape[1])
							...
							leg = plt.legend(loc="lower left", fontsize = 8)
							leg.get_frame().set_alpha(0.5)
							plt.suptitle('{1} {0} {2}'.format(g, names[oi], dc['Flylinename']))
							plt.savefig(ensure_dir("plots/mt/tbin_{0}_{1}_{2}_{3}_{4}.pdf".format(li, dc['Flylinename'], g, oi, bi)))
							plt.clf()
		...
		for dc in self.data_containers:
			...
			for i, g in enumerate(dc['Genename']):
				...
				for bi in xrange(n_bins):
					...
					plt.savefig(ensure_dir("plots/mt/slope_{0}_{1}_{2}.pdf".format(dc['Flylinename'], g, bi)))
					plt.clf()
	...
	def mv_pca(self, di=20):
		for dc in self.data_containers:
			...
			for bi in xrange(n_bins):
				...
				plt.savefig(ensure_dir("plots/mt/mv_pca_{0}_{1}.pdf".format(dc['Flylinename'], bi)))
				plt.clf()
				plt.subplot(411)
				...
				plt.savefig(ensure_dir("plots/mt/mv_tpca_{0}.pdf".format(dc['Flylinename'])))
				plt.clf()

if __name__ == '__main__':
	sc = ScalingContainerMutant()
	sc.length_binning()
	#sc.pca_gregor()
	#sc.time_plot()