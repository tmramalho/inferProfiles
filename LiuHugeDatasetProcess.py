...
class LiuHugeDataset(object):
	...
	def fitLogExponentials(self, quality = 1, nump = 50, length=False):
		...
		plt.imshow(hist.T, interpolation = 'nearest', cmap=cm.Blues, aspect = 'auto',
				...
				origin='lower')
		plt.grid(False)
		plt.savefig('results/lambdaLog.png')
	...
	def correlateExponentialsLength(self):
		...
		plt.title("R^2:{0:.2f}, pval:{1:.2e}".format(r_value**2, p_value))
		plt.subplot(313)
		slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(la[:, 2], la[:,1]/(la[:, 2]/500.0))
		print slope, intercept, r_value, p_value, std_err
		plt.scatter(la[:, 2], la[:,1]/(la[:, 2]/500.0))
		plt.plot(x, slope*x+intercept, color='r')
		plt.title("R^2:{0:.2e}, pval:{1:.2e}".format(r_value**2, p_value))
		plt.show()
